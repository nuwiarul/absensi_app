...
function recapKindFromRow(r: any, tz: string): { kind: RecapKind; label: string; detail?: string } {
  // Holiday row (from working_days) - no attendance
  if (r?.day_type === "HOLIDAY") return { kind: "HOLIDAY", label: "Holiday" }

  // Today pending
  if (r?.is_pending_today) return { kind: "BELUM_ABSEN", label: "Belum Absen" }

  // Missing / partial
  if (r?.is_missing_in) return { kind: "MISSING_IN", label: "Masuk Tidak Absen" }
  if (r?.is_missing_out) return { kind: "MISSING_OUT", label: "Pulang Tidak Absen" }
  if (r?.is_missing) return { kind: "TANPA_ABSEN", label: "Tanpa Absen" }

  // Early out
  if (r?.is_early_out) {
    const mins = r?.early_out_minutes
    return { kind: "PULANG_CEPAT", label: "Pulang Cepat", detail: mins ? `${mins} menit lebih awal` : undefined }
  }
  ...
  if (lt && lt !== "NORMAL") {
    ...
    return { kind: lt as any, label: lbl, detail: notes }
  }
...
export default function AttendanceRecapPage() {
  ...
  const mergedRows: RecapRowEx[] = React.useMemo(() => {
    ...
    // Generate rows for calendar WORKDAY/HALF_DAY even if attendance is empty.
    ...
    for (const [date, w] of wdByDate.entries()) {
      ...
      if (w.day_type === "HOLIDAY") continue
    ...
    if (out.length === 0) {
      ...
      return (rows as RecapRowEx[])
        ...
        .map((r) => {
          ...
          const isMissingIn = isPast && !hasNonNormalLeave && !r.check_in_at && !!r.check_out_at
          const isMissingOut = isPast && !hasNonNormalLeave && !!r.check_in_at && !r.check_out_at
          return { ...r, is_missing_in: isMissingIn, is_missing_out: isMissingOut }
        })
    }
    ...
    return out
  }, [from, to, rows, workingDays, leaveRows, satkerId, userId, selectedUser?.full_name, selectedUser?.nrp, selectedSatker?.name, selectedSatker?.code, todayYmd, showToday])
